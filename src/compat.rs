...
// A C ABI cannot be expressed in safe Rust: every entry point takes raw
// pointers supplied by a C caller. The `unsafe` here is confined to this
// module -- the engine itself is `unsafe_code = "deny"` -- and each function
// documents the invariants the caller must uphold.
#![allow(unsafe_code)]
