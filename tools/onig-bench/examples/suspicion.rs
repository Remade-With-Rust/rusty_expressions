...
fn main() {
    ...
    fn always_fail(_c: &rusty_expressions::CalloutCtx<'_>) -> CalloutResult {
        CalloutResult::Fail
    }
    fn always_skip(_c: &rusty_expressions::CalloutCtx<'_>) -> CalloutResult {
        CalloutResult::Skip
    }
    fn always_ok(_c: &rusty_expressions::CalloutCtx<'_>) -> CalloutResult {
        CalloutResult::Success
    }
    for (name, hook) in [
        ("Fail", always_fail as fn(&rusty_expressions::CalloutCtx<'_>) -> CalloutResult),
        ("Skip", always_skip),
        ("Success", always_ok),
    ] {
        for (pat, hay) in [
